	...
	def __fileTemplateExistsOrImportFromRemote(filepath):
		...
		if os.path.exists(OXA_FILEHD_REMOTE_TEMPLATES+path):
			...
			try:
				...
				if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7","/bin/cp",OXA_FILEHD_REMOTE_TEMPLATES+path, OXA_FILEHD_CACHE_TEMPLATES+path], preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
		...
		if FileHdManager.__fileTemplateExistsOrImportFromRemote(vm.xen_configuration.hd_origin_path):
			...
			try:
				...
				if OXA_FILEHD_CREATE_SPARSE_DISK:
					...
					if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7", "/bin/dd","if=/dev/zero","of="+vm_path,"bs=1M","count=1","seek="+str(size)], preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
						print "Failed to create Disk"
						raise Exception("")
				else:
					if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7","/bin/dd","if=/dev/zero","of="+vm_path,"bs=1M","count="+str(size)], preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
						print "Failed to create Disk"
						raise Exception("")
				
				#Create Swap
				if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7","/bin/dd","if=/dev/zero","of="+swap_path,"bs=1M","count=512"], preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
					print "Failed to create Swap"
					raise Exception("")
				#Format
				print "Creating EXT3 fs..."
				if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7","/sbin/mkfs.ext3","-F","-q",vm_path], preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
				...
				if subprocess.call(["/usr/bin/ionice", "-c", "2", "-n", "7","/bin/tar","-xvf",template_path,"-C",path],stdout=open(os.devnull, 'w'), preexec_fn=lambda : os.nice(OXA_FILEHD_COPY_OPERATIONS_NICE_PRIORITY)) > 0:
