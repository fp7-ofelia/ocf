...
class SessionTypeMixin(object):
    ...
    def __eq__(self, obj):
        ...
        if (
            ...
            obj.get_encoded == self.get_encoded and
...
class sessionType(GeneratedsSuper, SessionTypeMixin):
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='sessionType'):
        ...
        if self.get_encoded is not None:
            showIndent(outfile, level)
            outfile.write('<%sget_encoded>%s</%sget_encoded>\n' % (namespace_, self.gds_format_boolean(self.gds_str_lower(str(self.get_encoded)), input_name='get_encoded'), namespace_))
