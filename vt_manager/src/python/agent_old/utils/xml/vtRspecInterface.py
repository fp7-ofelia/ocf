...
class resources_type(GeneratedsSuper):
    ...
    def add_servers(self, value): self.servers.append(value)
    def insert_servers(self, index, value): self.servers[index] = value
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='resources-type'):
        for servers_ in self.servers:
            servers_.export(outfile, level, namespace_, name_='servers')
    def hasContent_(self):
        if (
            self.servers
            ):
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        showIndent(outfile, level)
        outfile.write('servers=[\n')
        level += 1
        for servers_ in self.servers:
            showIndent(outfile, level)
            outfile.write('model_.server_type(\n')
            servers_.exportLiteral(outfile, level, name_='server-type')
            showIndent(outfile, level)
            outfile.write('),\n')
        level -= 1
        showIndent(outfile, level)
