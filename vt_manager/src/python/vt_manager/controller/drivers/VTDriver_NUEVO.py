...
class VTDriver():
	...
	@staticmethod
	def addNetorkInterfaces(request, instance):
		from vt_manager.models.XenServer import XenServer
		server = XenServer.objects.get(uuid = instance.getUUID())
		server.addDataBridge(HttpUtils.getFieldInPost(request,NetworkInterface, "name"),
							macStr,switchId,port)

	@staticmethod
	def createOrUpdateServerFromPOSTInstance(instance):
		from vt_manager.models.VTServer import VTServer
		controller = VTDriver.getDriver(instance.getVirtTech())
		return controller.createOrUpdateServerFromPOSTInstance(instance)

	@staticmethod
	def createOrUpdateNetorkInterfacesFromPOSTInstance(server, ifaces):
		from vt_manager.models.VTServer import VTServer
		controller = VTDriver.getDriver(server.getVirtTech())
		return controller.createOrUpdateNetorkInterfacesFromPOSTInstance(server,ifaces)
