...
class IPallocator():
    ...
    def acquire(serverID, projectID, amID, sliceID, vmID, ifaceName, isMgmt):
        ...
        #XXX: This attribute is not used any more, should be removed
        ip.setAMslotID(IPallocator.MIN_HOST)
        ...
        #from the MIN_HOST to the MAX present, look for one is free
            for i in range(IPallocator.MIN_HOST, max(Ip.objects.values('hostID'))['hostID']+1):
                if not Ip.objects.filter(hostID = i):
                    ip.setHostID(i)
                    break
            #if the index i is equal to the max host, it is because there was none free, so host should be the following
            if i == max(Ip.objects.values('hostID'))['hostID'] :
                ip.setHostID(i+1)


        ip.setIp(IPallocator.ipString(server.ipRange, server.getMask(), ip.hostID))
        ip.save()
        IPallocatorMutex.release()
        return ip
    ...
    @staticmethod
    def ipString(serverRange, netmask, hostID):
        ...
        for i, nm in enumerate(n):
            if n[3-i] != '0':
                nb = MACallocator.Denary2Binary(int(n[3-i]), 8)
                for j, nbb in enumerate(nb):
                    if str(nb[7-j]) != '0':
                        lastBits = 8-j
                        break
                netmaskBits = lastBits + (3-i)*8
                break
        s = str(serverRange)
        s = s.split('.')
        prefix = ''
        ...
        prefix = prefix[:netmaskBits]
        add = prefix + MACallocator.Denary2Binary(hostID, 32 - netmaskBits)
        add2 = []
        for i in range(4):
