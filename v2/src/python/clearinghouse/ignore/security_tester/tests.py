...
class TestAll(TestCase):
    ...
    def setUp(self):
        ...
        self.user_test_role = TestModelRole.objects.create(
            security_user=self.user,
            security_object=self.user_test_model,
    ...
    def test_ownership(self):
        ...
        for r in self.owner.security_testmodel_roles.all():
            self.assertEqual(r.security_role_choice, 'Owner',
                             'Owner has non-ownership role!')
    ...
    def test_protect_roleless_obj(self):
        ...
        seen_set = set(TestModel.objects.all().values_list('pk', flat=True))
        wanted_set = set([self.admin_test_model.pk, self.user_test_model.pk])
        self.assertEqual(seen_set, wanted_set,
                         'Saw %s. Expected %s' % (seen_set, wanted_set))
        
        self.assertRaises(TestModel.DoesNotExist,
    ...
    def test_nonsecret_reads(self):
        ...
        obj = TestModel.objects.get(pk=self.admin_test_model.pk)
        self.assertEqual(self.admin_test_model.nonsecret,
                         obj.nonsecret,
                         'Did not get correct value for non-secret field')

        obj = TestModel.objects.get(pk=self.user_test_model.pk)
    ...
    def test_allowed_limited_write(self):
        ...
        self.assertEqual(45,
                         ...
                         'Did not store the correct value' 
                         + 'for limitedwriteable field.')
    ...
    def test_disallowed_limited_write(self):
        ...
        self.assertRaises(TestModel.SecurityException,
