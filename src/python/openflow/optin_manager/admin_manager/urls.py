...
urlpatterns = patterns('openflow.optin_manager.admin_manager.views',
    ...
    url(r'^approve_user/(?P<req_id>\d+)/approve$', 'approve_user',{'operation':1}, name="approve_user_reg"),
    url(r'^approve_user/(?P<req_id>\d+)/reject$', 'approve_user',{'operation':2}),
    url(r'^approve_user$','approve_user',{'operation':0,'req_id':0}, name="approve_user_reg"),
