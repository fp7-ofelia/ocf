...
public class BlockArrow implements Shape {
	...
	public BlockArrow(Rectangle r0, Rectangle r1) {
		this(r0, r1, 0);
	}
	
	public BlockArrow(double x0, double y0, double x1, double y1) {
		updateShape((int)x0, (int)y0, (int)x1, (int)y1, 0);
	}

	public BlockArrow(int x0, int y0, int x1, int y1) {
		updateShape(x0, y0, x1, y1, 0);
	}

	public BlockArrow(Rectangle r0, Rectangle r1, int offset) {
        ...
        updateShape((int)p1.x, (int)p1.y, (int)p2.x, (int)p2.y, offset);
	}

	public void updateShape(int x0, int y0, int x1, int y1, int offset) {
		double dx = x1-x0;
		double dy = y1-y0;
		...
		pts[0] = new Point(x0, y0 - BODY_WIDTH/2 - offset);
		pts[1] = new Point(pts[0].x + length - HEAD_LENGTH, pts[0].y);
		pts[2] = new Point(pts[1].x, pts[1].y - (HEAD_WIDTH - BODY_WIDTH)/2);
		pts[3] = new Point(x0 + length, y0 - offset);
		pts[6] = new Point(x0, y0 + BODY_WIDTH/2 - offset);
		pts[5] = new Point(pts[6].x + length - HEAD_LENGTH, pts[6].y);
		pts[4] = new Point(pts[5].x, pts[5].y + (HEAD_WIDTH - BODY_WIDTH)/2);
