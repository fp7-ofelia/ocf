    ...
    if request.method == "POST":
        ...
        if agg.check_status() == False:
            DatedMessage.objects.post_message_to_user(
                "Sample Resource Aggregate '%s' is not available" % agg.name,
        ...
        if 'create_resource' in request.POST:
            ...
            return HttpResponseRedirect(reverse("resource_crud",
                                                args=[slice_id,server_id]))
        else:
            return HttpResponseRedirect("/")

def resource_crud(request, slice_id, server_id):
...
#                   #("Resource visualization panel ", reverse("slice_home", args=[slice_id])),
                    ("Create VM in server %s" %serv.name, reverse("resource_crud", args=[slice_id, server_id])),
                )
        })
...
#            openflowSwitches[s.datapath_id] = len(nodes_test)-1
    ...
    for i, agg in enumerate(sr_aggs):
        ...
        for n in vt_servers:
            ...
            nodes.append(dict(
                    ...
                    description = get_node_description(n), image = image_url,
                    available = agg.available, vmInterfaces = vmInterfaces, loc = agg.location)
            )
...
#        ui_context['nodes'], ui_context['links'] = [[{"value":"249", "image": ""}], []]
...
#        sr_aggs = get_vm_aggregates(slice)
#        ui_context['nodes'], ui_context['links'], ui_context['n_islands'] = _get_nodes_links(of_aggs, pl_aggs, sr_aggs, slice.id)
...
#        ui_context = dict()
#        try:
#            ui_context['sr_list'] = get_resource_list(slice)
#            ui_context['sr_aggs'] = get_sr_aggregates(slice)
#            ui_context['nodes'], ui_context['links'] = get_nodes_links(slice)
#        except Exception as e:
#            print "[ERROR] Problem loading UI data for plugin 'vt_plugin'. Details: %s" % str(e)
