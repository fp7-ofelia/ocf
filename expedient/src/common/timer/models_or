...
class JobManager(models.Manager):
    def schedule_post_syncdb(self, period, callable):
        ...
        uid = "schedule_post_syncdb_job_%s_%s" % (period, callable.__name__)

        def schedule_callback_factory():
            def schedule_callback(sender, **kwargs):
                try:
                    self.schedule(period, callable)
    ...
    def schedule(self, period, callable):
        ...
        callable_name = stringify_func(callable)
    ...
    def unschedule(self, callable):
        ...
        try:
            ...
            raise JobNotScheduled(callable)
        
        job.delete()
        
    def get_for(self, callable):
...
class Job(models.Model):
    ...
    def execute_now(self):
        ...
        callable = get_callable(self.callable_name)
        return callable()
