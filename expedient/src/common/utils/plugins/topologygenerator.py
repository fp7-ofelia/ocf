...
#				print "[WARNING] Problem sending e-mail to user '%s' (email: %s) with information about link inconsistency inside TopologyGenerator. Details: %s" % (user.username, user.email, str(e))
			try:
				...
				DatedMessage.objects.post_message_to_user(
					"Inconsistent links at '%s'>'%s': %s" % (slice.project.name, slice.name, str(inconsistent_links)),
					user, msg_type=DatedMessage.TYPE_WARNING)
			except Exception as e:
				print "[WARNING] Problem with DatedMessage sent to user '%s' with information about link inconsistency inside TopologyGenerator. Details: %s" % (user.username, str(e))
