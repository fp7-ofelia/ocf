"""
Controller for the GeniApi object.
...
from geni_api.utils.validators import validate_resource_name
import decimal, random

class GeniApi():
    """
    Manages creation/edition of GeniApis from the input of a given form.
    """
    ...
    def create(instance, aggregate_id, slice=None):
        try:
            ...
            if "Duplicate entry" in str(e):
                raise Exception("GeniApi with name '%s' already exists." % str(instance.name))
            else:
                raise e
    ...
    def fill(instance, slice, aggregate_id, resource_id = None):
        ...
        if not resource_id:
            if GeniApiModel.objects.filter(name = instance.name):
    ...
    def delete(resource_id):
        try:
            GeniApiModel.objects.get(id = resource_id).delete()
        except:
            pass
