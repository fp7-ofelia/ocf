...
class XMLMarshal(object):
    ...
    def dump(stream, xmlfile):
        ...
        try:
            p=make_parser()
            print "parser: %s" % str(p)
            h = XMLDictionaryHandler()
            print "handler: %s" % str(h)
            p.setContentHandler(h)
            p.parse(open(xmlfile))
            print "parser 2: %s" % str(p)
            # print h.d
            print "handler stream: %s" % str(h.stream)
            marshal.dump(h.d, stream)
            print "dump.stream: %s" % str(stream)
        except Exception, e:
            sys.exit(e)
    ...
    def load(stream, out=sys.stdout):
        ...
        try:
            ...
            p.parse(stream)
            print "load.stream: %s" % str(stream)
        except Exception, e:
            sys.exit(e)
...
if __name__ == '__main__':
    ...
    xmlSchema = etree.XMLSchema(open(fname))
    
#    p=make_parser()
#    print "parser1111111111111: %s" % str(p)
#    h = XMLDictionaryHandler()
#    print "handler111111111111: %s" % str(h)
#    p.setContentHandler(h)
#    print "handler22222222222222: %s" % str(p.getContentHandler())
#    p.parse(open(fname))
#    print "handler stream11111111: %s" % str(h.stream)

    # Dump XML text to binary
    XMLMarshal.dump(open(binname,'wb'), fname)
    # Dump XML binary to text
    XMLMarshal.load(open(binname,'rb'), open(fname,'w'))
