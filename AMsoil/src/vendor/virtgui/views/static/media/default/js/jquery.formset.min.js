...
return false;});});if($(this).length){var $addBtn;if($(this).attr('tagName')=='TR'){var numCols=this.eq(0).children().length;$(this).parent().append('<tr><td colspan="'+numCols+'"><input type="button" value="'+options.addText+'" onClick="document.location.href=\'javascript:void(0)\';"></tr>');$addBtn=$(this).parent().find('tr:last a');}else{var numCols=7;$(this).filter(':last').after('<tr class="right"><td colspan="'+numCols+'"><input type="button" value="'+options.addText+'" id="'+options.addText+'" onClick="'+options.onClickFunction+';"></td></tr>');$addBtn=$(this).filter(':last').next();}
$addBtn.click(function(){var nextIndex=parseInt($('#id_'+options.prefix+'-TOTAL_FORMS').val());var row=$('.'+options.formCssClass+':first').clone(true).get(0);$(row).removeAttr('id').insertAfter($('.'+options.formCssClass+':last'));$(row).find('td').each(function(){$(this).removeClass()});$(row).find('input,select,textarea,label').each(function(){updateElementIndex(this,options.prefix,nextIndex);var elem=$(this);if(elem.is('input:checkbox')||elem.is('input:radio')){elem.attr('checked',false);}else{elem.val('');elem.attr('readonly',false);}});var formCount=nextIndex+1;$('#id_'+options.prefix+'-TOTAL_FORMS').val(formCount);if(formCount>1){$('a.'+options.deleteCssClass).show();}
if(options.added)options.added($(row));return false;});}
if($(this).length==1){$('a.'+options.deleteCssClass).hide();}
return $(this);}
$.fn.formset.defaults={prefix:'form',addText:'add another',deleteText:'remove',addCssClass:'add-row',deleteCssClass:'delete-row',formCssClass:'dynamic-form',added:null,removed:null,onClickFunction:'document.location.href=\'javascript:void(0)\''}})(jQuery)
