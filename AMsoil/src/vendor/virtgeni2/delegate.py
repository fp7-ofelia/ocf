...
GENIv2DelegateBase = pm.getService('geniv2delegatebase')
geniv2_exception = pm.getService('geniv2exceptions')
virt_exception = pm.getService('virtexceptions')

class VTDelegate(GENIv2DelegateBase):
    """
    ...
    def get_request_extensions_mapping(self):
        """Documentation see [geniv2rpc] GENIv2DelegateBase."""
    ...
    def list_resources(self, client_cert, credentials, geni_available):
        ...
        for server in servers:
            if int(server.get_available()) is 0 and geni_available: 
                ...
                n.append(E.cpus_number("0" if not server.get_number_of_cpus() else server.get_number_ofcpus()))
    ...
    def allocate(self, slice_urn, client_cert, credentials, rspec, end_time=None):
        ...
        for nodes in rspec_root.getchildren():
            if not nodes.tag == "node":
                raise geniv2_exception.GENIv2BadArgsError("RSpec contains elements/namespaces I do not understand (%s)." % (nodes,))        
            for slivers in nodes.getchildren():
                vm = dict()
                if not slivers.tag == "sliver":
                    raise geniv2_exception.GENIv2BadArgsError("RSpec contains elements/namespaces I do not understand (%s)." % (slivers,))
                for elm in slivers.getchildren():
                    if (elm.tag == "name"):
                        ...
                        raise geniv2_exception.GENIv2BadArgsError("RSpec contains an element I do not understand (%s)." % (elm,))
        ...
        if self.urn_type(slice_urn) is 'slice': 
            try:
                ...
                raise geniv2_exception.GENIv2AlreadyExistsError("The desired VM name(s) is already taken (%s)." % (requested_vms[0]['name'],))
            except virt_exception.VTAMServerNotFound as e:
                self.undo_action("allocate", allocated_vms)
                raise geniv2_exception.GENIv2SearchFailedError("The desired Server name(s) cloud no be found (%s)." % (requested_vms[0]['server_name'],))
            except virt_exception.VTMaxVMDurationExceeded as e:
                self.undo_action("allocate", allocated_vms)
                raise geniv2_exception.GENIv2BadArgsError("VM allocation can not be extended that long (%s)" % (requested_vm['name'],))
        else:
            raise geniv2_exception.GENIv2OperationUnsupportedError('Only slice URNs are admited in this method for this AM')
    ...
    def renew(self, urns, client_cert, credentials, expiration_time, best_effort):
        ...
        if self._resource_manager.check_expiration_time(expiration_time) is False:
            raise geniv2_exception.GENIv2BadArgsError("VMs can not be extended that long")
        vms = list()
        expirations = list()
        for urn in urns:
    ...
    def status(self, urns, client_cert, credentials):
        ...
        for urn in urns:
            if (self.urn_type(urn) == 'slice'):
                ...
                slice_vms = self._resource_manager.get_vms_in_slice(urn)
                if not slice_vms:
                    raise geniv2_exception.GENIv2SearchFailedError("There are no resources in the given slice(s)")
                for vm in slice_vms:
                ...
                try:
                    ...
                    raise geniv2_exception.GENIv2SearchFailedError("The desired VM urn(s) cloud not be found (%s)." % (vm,))
            else:
                raise geniv2_exception.GENIv2OperationUnsupportedError('Only slice or sliver URNs can be given to status in this aggregate')
            ...
            if (self.urn_type(urn) == 'slice'):
                ...
                for vm in slice_vms:
                    if (action == self.OPERATIONAL_ACTION_START):
                        ...
                        raise geniv2_exception.GENIv2BadArgsError("Action not suported in this AM" % (urn,))
            elif (self.urn_type(urn) == 'sliver'):     
                if (action == self.OPERATIONAL_ACTION_START):
                ...
                raise geniv2_exception.GENIv2OperationUnsupportedError('Only slice or sliver URNs can be given in this aggregate')
        sliver_list = list()
        for vm in vms:
            if vm.has_key('error'):
    ...
    def delete(self, urns, client_cert, credentials, best_effort):
        ...
        for urn in urns:
            if (self.urn_type(urn) != "slice") and (self.urn_type(urn) != "sliver"):
                raise geniv2_exception.GENIv2OperationUnsupportedError('Only slice and sliver URNs can be deleted in this aggregate') 
    ...
    def shutdown(self, slice_urn, client_cert, credentials):
        if (self.urn_type(slice_urn) == 'slice'):
            ...
            raise geniv2_exception.GENIv2OperationUnsupportedError('Only slice URNs can be given to shutdown in this aggregate')
