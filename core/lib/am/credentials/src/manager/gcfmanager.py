from ambase.src.abstract.classes.credentialmanagerbase import \
...
class GCFCredentialManager(CredentialManagerBase):
    ...
    def _get_geniv2_validation(self, method, credentials, caller_cert=None):
        ...
        try:
            # UT: speaks-for
            # cert = self.__root_cert
            cert = caller_cert if caller_cert else self.__root_cert
            valid_cred = self.__auth.verify_from_strings(
                cert, credentials, None, method, {})
            return valid_cred
